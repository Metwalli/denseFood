...
def resize_test_images(origin_dir, test_dir, output_dir):
    classes_list = os.listdir(test_dir)

    # Preprocess train, dev and test
    idx = 0
    for c in classes_list:
        output_dir_split = os.path.join(output_dir, c)
        if not os.path.exists(output_dir_split):
            os.mkdir(output_dir_split)
        else:
            print("Warning: dir {} already exists".format(output_dir_split))
        print("Processing {} data, saving preprocessed data to")
        class_image_paths = sorted(list(paths.list_images(os.path.join(test_dir, c))))
        for filename in tqdm(class_image_paths):
            image_name = filename.split(os.path.sep)[-1]
            origin_filename = os.path.join(origin_dir, c, image_name)
            resize_and_save(origin_filename, output_dir_split, size=300)


if __name__ == '__main__':
    # origin_dir = "C:\data\VIREO\\food-10"
    # test_dir = "C:\data\VIREO\\10\\eval"
    # output_dir = "C:\data\VIREO\\11"
    # resize_test_images(origin_dir, test_dir, output_dir)
